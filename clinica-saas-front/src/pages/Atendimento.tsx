...
import { MessageSquare, Inbox, ArrowLeftRight, Send, User, Bot, CheckCircle, ArrowLeft } from 'lucide-react';
...
export default function Atendimento() {
  ...
  return (
    // Ajuste de altura para mobile não dar barra de rolagem dupla
    <div className="flex h-[calc(100vh-8rem)] md:h-[85vh] bg-white md:rounded-3xl shadow-sm md:border border-gray-100 overflow-hidden -mx-4 md:mx-0 mt-2 md:mt-0">
      ...
      {/* 📱 Mobile: Esconde se tiver conversa aberta / 💻 PC: Sempre aparece */}
              ...
              >
                <div className="flex justify-between items-center mb-1">
                  <h4 className="font-bold text-gray-900 truncate flex items-center gap-2 text-sm md:text-base">
                    ...
                    <span className="truncate">{conv.contacts?.name || conv.contacts?.phone_number}</span>
                  </h4>
                  {conv.unread_count > 0 && (
      ...
      {/* JANELA DE CHAT */}
      {/* 📱 Mobile: Esconde se NÃO tiver conversa aberta / 💻 PC: Sempre aparece */}
            ...
            <div className="h-16 md:h-20 bg-white border-b border-gray-100 flex items-center justify-between px-3 md:px-8 shadow-sm z-10 shrink-0">
              <div className="flex items-center gap-2 md:gap-4 truncate">
                ...
                <div className="truncate">
                  <h2 className="text-base md:text-lg font-bold text-gray-900 truncate">{conversaSelecionada.contacts?.name}</h2>
                  <p className="text-xs md:text-sm text-gray-500 truncate">{conversaSelecionada.contacts?.phone_number}</p>
                </div>
              </div>
              
              {/* Botão Resolver - Em telas muito pequenas vira só o ícone */}
              {abaAtiva === 'transferidas' && (
                <button onClick={encerrarAtendimento} className="flex items-center gap-1 md:gap-2 bg-green-50 text-green-700 px-3 md:px-4 py-2 rounded-xl font-bold hover:bg-green-100 transition shrink-0 ml-2">
